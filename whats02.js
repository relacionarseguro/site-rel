...
// Seletores Popups de 90s (Relatório Final)
...
/**
 * 📢 FUNÇÃO MODIFICADA: Calcula a contagem de resultados baseada no progresso da barra.
 * @param {number} progressoSegundos Tempo atual do carregamento (0 a 90).
 */
function iniciarBuscaRapida(progressoSegundos) { 
    
    // Constantes de resultados FINAIS
    const maxConversas = 5; 
    const maxFotos = 8;     
    const maxAudios = 3; 
    
    // Duração total da barra de carregamento do relatório (em segundos)
    const DURACAO_TOTAL = 90; 

    // O progresso de 0 a 1 (percentual)
    const progressPercent = progressoSegundos / DURACAO_TOTAL; // 0 a 1

    // --- 1. CÁLCULO DE FOTOS (Máximo de 8 em 75%) ---
    const FOTOS_THRESHOLD = 0.75; 
    let countFotos;

    if (progressPercent <= FOTOS_THRESHOLD) {
        // Crescimento proporcional até 75%
        countFotos = Math.floor((progressPercent / FOTOS_THRESHOLD) * maxFotos);
    } else {
        // Mantém o máximo
        countFotos = maxFotos;
    }
    
    // Garantir que a contagem comece em 1, a menos que o tempo seja 0
    countFotos = (progressoSegundos > 0) ? Math.max(1, countFotos) : 0; 
    

    // --- 2. CÁLCULO DE CONVERSAS (Máximo de 5 em 80%) ---
    const CONVERSAS_THRESHOLD = 0.80; 
    let countConversas;

    if (progressPercent <= CONVERSAS_THRESHOLD) {
        // Crescimento proporcional até 80%
        countConversas = Math.floor((progressPercent / CONVERSAS_THRESHOLD) * maxConversas);
    } else {
        countConversas = maxConversas;
    }
    
    // Garantir que a contagem comece em 1, a menos que o tempo seja 0
    countConversas = (progressoSegundos > 0) ? Math.max(1, countConversas) : 0;


    // --- 3. CÁLCULO DE ÁUDIOS RECUPERADOS (Máximo de 3 em 80%) ---
    const AUDIOS_THRESHOLD = 0.80; 
    let countAudios = 0;

    if (progressPercent <= AUDIOS_THRESHOLD) {
        // Crescimento proporcional
        countAudios = Math.floor((progressPercent / AUDIOS_THRESHOLD) * maxAudios);
    } else {
        countAudios = maxAudios;
    }
    
    // --- 4. CÁLCULO DAS FRASES DE ARQUIVOS (Dinâmica) ---
    // A frase de áudios é exibida apenas no final do ciclo de frases
    const frasesArquivosEstaticas = [
...
    ];
    const fraseAudios = `áudios recuperados (${countAudios}) deletados.`;

    // A frase muda a cada 0.5 segundos (500ms), sincronizado com o setInterval
    const indiceBase = Math.floor(progressoSegundos / 0.5); 
    const fraseIndex = indiceBase % (frasesArquivosEstaticas.length + 1);
    
    let fraseAtual;
    if (fraseIndex < frasesArquivosEstaticas.length) {
        fraseAtual = frasesArquivosEstaticas[fraseIndex];
    } else {
        fraseAtual = fraseAudios;
    }


    // --- 5. ATUALIZAÇÃO DO TEXTO NA TELA ---
    if (progressoSegundos >= DURACAO_TOTAL) {
         // Estado final (Busca CONCLUÍDA)
        ...
        if (outputArquivos) outputArquivos.textContent = `Busca de Arquivos CONCLUÍDA. ${fraseAudios}`;
        
    } else {
        // Estado em carregamento (Dinâmico)
        if (outputFotos) {
        ...
        if (outputArquivos) {
            outputArquivos.textContent = fraseAtual;
        }
    }
}
...
    relatorioLoadingCounter.textContent = `Carregando: ${count}s`;
    ...
    // NOVO: Intervalo para atualizar os números de fotos/conversas rapidamente
    const intervalBuscaRapida = setInterval(() => {
        // Passa o tempo atual da barra para a função de busca
        iniciarBuscaRapida(count); 
    }, 100); // Roda a cada 100ms (10 vezes por segundo)

    // Intervalo principal para a barra de tempo (a cada 1 segundo)
    const intervalId = setInterval(() => {
        count++;
        relatorioLoadingCounter.textContent = `Carregando: ${count}s`;

        // A barra de carregamento usa 90 como divisor
        const progress = (count / 90) * 100; 
        
        relatorioLoadingBarFill.style.transition = 'width 1s linear';
        relatorioLoadingBarFill.style.width = `${progress}%`;

        // Condição de parada (90 segundos)
        if (count >= 90) { 
            clearInterval(intervalId); 
            clearInterval(intervalBuscaRapida); // Limpa o novo intervalo
            
            // Garante que o estado final CONCLUÍDO seja exibido
            iniciarBuscaRapida(90); 
